...
# leksykograficznym k-elementowych podzbiorów zbioru {1,...,n}.
...
class SubsetFinder:
    ...
    def unrank(self):
        ...
        for i in range(1, self.k + 1):
            while (self.binomial(self.n - candidate, self.k - i) <= r and candidate < self.n):
                r -= self.binomial(self.n - candidate, self.k - i)
                candidate += 1
